...
bool GVPixmap::save() const {
	return saveAs(url(),mMimeType);
}


bool GVPixmap::saveAs(const KURL& url, const QString& mMimeType) const {
	QString format=KImageIO::typeForMime(mMimeType);
...
bool GVPixmap::load() {
	...
	mMimeType=KImageIO::mimeType(path);
